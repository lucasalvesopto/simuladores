...
// Variáveis do Paciente (Método de Sheard / AA)
// OBS: Futuramente, criaremos um sorteio para a 'idade', 'estadoAccOD' e 'estadoAccOE'.
// Por enquanto, os valores estão fixos para facilitar os testes.
let idade = 40;
let aaEsperado = 15 - (idade / 4); // Fórmula da Amplitude Esperada
let estadoAccOD = 5; // Valor que será sorteado para o Olho Direito
let estadoAccOE = 5; // Valor que será sorteado para o Olho Esquerdo
let recobroOD = 1.0; // Valor de recobro (0.00 a 1.00D) que será sorteado
let recobroOE = 0.50;
let dxAccOD = '';
let dxAccOE = '';
...
let borrouJ2_OD = false; // Flag se o olho direito atingiu a fadiga
let borrouJ2_OE = false; // Flag se o olho esquerdo atingiu a fadiga
...
function gerarPaciente() {
    ...
    if (storedData) {
        try {
            ...
            if (data.idade !== undefined) {
                idade = data.idade;
                aaEsperado = 15 - (idade / 4);
            }
            if (data.amplitude) {
                estadoAccOD = data.amplitude.estadoAccOD ?? estadoAccOD;
                estadoAccOE = data.amplitude.estadoAccOE ?? estadoAccOE;
                recobroOD = data.amplitude.recobroOD ?? recobroOD;
                recobroOE = data.amplitude.recobroOE ?? recobroOE;
    ...
    const variacaoOD = (Math.random() * 4) - 2;
    const variacaoOE = (Math.random() * 4) - 2;

    estadoAccOD = Math.max(1.0, aaEsperado + variacaoOD);
    estadoAccOE = Math.max(1.0, aaEsperado + variacaoOE);

    recobroOD = parseFloat((Math.random() * 0.8 + 0.2).toFixed(2));
    recobroOE = parseFloat((Math.random() * 0.8 + 0.2).toFixed(2));

    atualizarDiagnostico();

    // Salva de volta para manter sincronizado com a Ficha Clínica
    const novoPaciente = {
        idade: idade,
        rxOD: { esf: rxEsfOD, cil: rxCilOD, eixo: rxEixoOD },
        rxOE: { esf: rxEsfOE, cil: rxCilOE, eixo: rxEixoOE },
        amplitude: {
            estadoAccOD: parseFloat(estadoAccOD.toFixed(2)),
            estadoAccOE: parseFloat(estadoAccOE.toFixed(2)),
            recobroOD: parseFloat(recobroOD.toFixed(2)),
            recobroOE: parseFloat(recobroOE.toFixed(2))
        }
    };
    try {
        localStorage.setItem('pacienteData', JSON.stringify(novoPaciente));
    } catch (e) {
        console.error("Erro ao salvar no localStorage", e);
    }
}

// Atualiza o diagnóstico com base na diferença (Tolerância de 0.25)
function atualizarDiagnostico() {
    let diferencaOD = estadoAccOD - aaEsperado;
    if (diferencaOD > 0.25) dxAccOD = 'Excesso de Acomodação';
    else if (diferencaOD < -0.25) dxAccOD = 'Insuficiência de Acomodação';
    else dxAccOD = 'Normal';

    let diferencaOE = estadoAccOE - aaEsperado;
    if (diferencaOE > 0.25) dxAccOE = 'Excesso de Acomodação';
    else if (diferencaOE < -0.25) dxAccOE = 'Insuficiência de Acomodação';
    else dxAccOE = 'Normal';
}
gerarPaciente();
...
function calcularVisao() {
    // Só avalia se pelo menos um olho estiver sendo testado (o outro ocluído)
    // Se nenhum estiver ocluído, usa o olho que tiver a melhor visão (menor esforço)
    let olhoAtivo = '';
    ...
    let excesso = 0;
    // A demanda visual a 33cm é de 3.00D.
    // Lentes negativas aumentam a demanda (ex: -1.00D aumenta para 4.00D)
    // Lentes positivas relaxam (ex: +1.00D diminui para 2.00D)
    const calcularExcesso = (lenteEsf, rxEsf, estadoAcc) => {
        let lenteEfetiva = lenteEsf - rxEsf;
        let demanda = 3.00 - lenteEfetiva;
        return demanda - estadoAcc;
    };

    if (olhoAtivo === 'od') {
        excesso = calcularExcesso(lenteEsfOD, rxEsfOD, estadoAccOD);
        // Lógica de Recobro (Histerese)
        if (excesso >= 0) {
            borrouJ2_OD = true; // Estourou o limite, ciliar fadigou
        } else if (borrouJ2_OD) {
            // Só recupera a visão se a folga for maior ou igual ao recobro
            if (excesso <= -recobroOD) borrouJ2_OD = false;
            else excesso = 0; // Continua borrado enquanto não recuperar!
        }
    } else if (olhoAtivo === 'oe') {
        excesso = calcularExcesso(lenteEsfOE, rxEsfOE, estadoAccOE);
        if (excesso >= 0) {
            borrouJ2_OE = true;
        } else if (borrouJ2_OE) {
            if (excesso <= -recobroOE) borrouJ2_OE = false;
            else excesso = 0;
        }
    } else {
        // Se ambos abertos, a visão é ditada pelo olho em melhor situação (menor excesso)
        let excOD = calcularExcesso(lenteEsfOD, rxEsfOD, estadoAccOD);
        if (excOD >= 0) borrouJ2_OD = true;
        else if (borrouJ2_OD && excOD <= -recobroOD) borrouJ2_OD = false;
        else if (borrouJ2_OD) excOD = 0;

        let excOE = calcularExcesso(lenteEsfOE, rxEsfOE, estadoAccOE);
        if (excOE >= 0) borrouJ2_OE = true;
        else if (borrouJ2_OE && excOE <= -recobroOE) borrouJ2_OE = false;
        else if (borrouJ2_OE) excOE = 0;
    ...
    // A variável "excesso" representa o quanto passamos do limite do paciente (que é onde o J2 borra).
    if (excesso < -1.00) {
        // Tem muita folga de acomodação. Vê perfeitamente J1.
        linhaAV = 1;
    } else if (excesso >= -1.00 && excesso < 0) {
        // J1 borrou (porque exige mais foco), mas ele ainda consegue ler J2!
        linhaAV = 2;

        // Quando chega a 0.50 ou 0.25 do limite de J2, ele pisca/esforça para manter o J2 nítido
        ...
        // excesso = 0.00 -> linhaAV = 3 (lê J3)
        // excesso = 0.50 -> linhaAV = 4 (lê J4)
        // excesso = 1.00 -> linhaAV = 5 (lê J5)
        linhaAV = 3 + Math.floor(excesso / 0.50);

        if (linhaAV > 7) linhaAV = 7; // Borrado total (Não lê nem J6)
    }
