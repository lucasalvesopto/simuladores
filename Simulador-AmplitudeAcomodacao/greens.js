// VariÃÂ¡veis de estado global do simulador
let isChartVisible = true;
let isGlassesVisible = false; // Ãâculos comeÃÂ§a na maleta
let currentLensZIndex = 11; // Controle dinÃÂ¢mico para a ÃÂºltima lente colocada ficar na frente

// RefraÃÂ§ÃÂ£o Final do Paciente (Futuro: VirÃÂ¡ do LocalStorage da Retinoscopia)
let rxEsfOD = 0.00;
let rxCilOD = 0.00;
let rxEixoOD = 0;
let rxEsfOE = 0.00;
...
// VariÃÂ¡veis do Paciente (MÃÂ©todo de Sheard / AA)
...
function calcularVisao() {
    ...
    let excesso = 0;
    const calcularExcesso = (lenteEsf, rxEsf, lenteCil, rxCil, lenteEixo, rxEixo, estadoAcc, ocluso) => {
        if (ocluso) return 0;
        ...
        if (cilErrado || eixoErrado) return 99;
    ...
    if (olhoAtivo === 'od') {
        excesso = calcularExcesso(esfOD_Efetivo, rxEsfOD, lenteCilOD, rxCilOD, lenteEixoOD, rxEixoOD, estadoAccOD, false);
        if (excesso >= 0 && excesso !== 99) borrouJ2_OD = true;
        else if (borrouJ2_OD && excesso !== 99) {
            if (excesso <= -recobroOD) borrouJ2_OD = false;
            else excesso = 0;
        }
    } else if (olhoAtivo === 'oe') {
        excesso = calcularExcesso(esfOE_Efetivo, rxEsfOE, lenteCilOE, rxCilOE, lenteEixoOE, rxEixoOE, estadoAccOE, false);
        if (excesso >= 0 && excesso !== 99) borrouJ2_OE = true;
        else if (borrouJ2_OE && excesso !== 99) {
            if (excesso <= -recobroOE) borrouJ2_OE = false;
            else excesso = 0;
        }
    } else {
        let excOD = calcularExcesso(esfOD_Efetivo, rxEsfOD, lenteCilOD, rxCilOD, lenteEixoOD, rxEixoOD, estadoAccOD, oclOD_Efetivo);
        if (excOD >= 0 && excOD !== 99) borrouJ2_OD = true;
        else if (borrouJ2_OD && excOD <= -recobroOD && excOD !== 99) borrouJ2_OD = false;
        else if (borrouJ2_OD && excOD !== 99) excOD = 0;

        let excOE = calcularExcesso(esfOE_Efetivo, rxEsfOE, lenteCilOE, rxCilOE, lenteEixoOE, rxEixoOE, estadoAccOE, oclOE_Efetivo);
        if (excOE >= 0 && excOE !== 99) borrouJ2_OE = true;
        else if (borrouJ2_OE && excOE <= -recobroOE && excOE !== 99) borrouJ2_OE = false;
        else if (borrouJ2_OE && excOE !== 99) excOE = 0;
